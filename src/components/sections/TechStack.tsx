...
const getIconUrl = (skill: string) => {
  ...
  const icon =
    mapping[skill] ||
    skill.toLowerCase().replace(/ /g, "").split("(")[0].split("&")[0].trim();
...
export function TechStack() {
  ...
  return (
    <section className="mt-16 py-10 px-6 max-w-xl mx-auto">
      <h2 className="text-center text-sm font-bold uppercase tracking-[0.2em] text-gray-400 dark:text-gray-500 mb-12">
              ...
              <img
                ...
                onError={(e) => {
                  (e.target as HTMLImageElement).src =
                    `https://ui-avatars.com/api/?name=${skill}&background=random&color=fff&size=48`;
                }}
              />
            </div>
            <span className="text-[10px] font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider group-hover:text-gray-900 dark:group-hover:text-white transition-colors text-center">
              {skill}
            </span>
          </motion.div>
        ))}
